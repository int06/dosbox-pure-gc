...
#if C_GAMELINK
#include "../gamelink/program_hash.h"
#endif // C_GAMELINK

const char * RunningProgram="DOSBOX";
#if C_GAMELINK
GameLink::ProgramHash RunningProgramHash;
#endif // C_GAMELINK
...
#if C_GAMELINK
	GameLink::ProgramHash loading_program_hash{ head.checksum };
#endif // C_GAMELINK
...
#if C_GAMELINK
		loading_program_hash.Append(loadbuf, readsize);
#endif // C_GAMELINK
...
//			if (readsize!=imagesize) LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
	...
	if ((flags==LOAD) || (flags==LOADNGO)) {
		...
		DOS_UpdatePSPName();
#if C_GAMELINK
		RunningProgramHash = loading_program_hash;
#endif // C_GAMELINK
	}
